...
dm$rank <-  rank(dm$t) - nrow(subset(dm,t<0))
mm <- merge(dm,gt,by.x=0,by.y="probe")
head(mm)
mma <-aggregate(mm$rank ~ gene,mm,mean)
rownames(mma) <- mma$gene
mma$gene = NULL
colnames(mma) <- "meanrank"
lrmres <- runmitch(m=mma,genesets=gs_symbols,cores=16)

lrmsig <- subset(lrmres,p.adjustANOVA<0.05)
...
lrmsigsets <- paste(rownames(lrmsig),as.character(lrmsig$s.dist > 0))
lrmsigsets2 <- gsub(" TRUE"," UP",lrmsigsets)
lrmsigsets2 <- gsub(" FALSE"," DN",lrmsigsets2)
...
mma$gene = NULL
colnames(mma) <- "meanrank"
lmrmres <- runmitch(m=mma,genesets=gs_symbols,cores=16)

lmrmsig <- subset(lmrmres,p.adjustANOVA<0.05)
...
# distinguish up (TRUE) and down (FALSE)
lmrmsigsets <- paste(rownames(lmrmsig),as.character(lmrmsig$s.dist > 0))
lmrmsigsets2 <- gsub(" TRUE"," UP",lmrmsigsets)
lmrmsigsets2 <- gsub(" FALSE"," DN",lmrmsigsets2)
