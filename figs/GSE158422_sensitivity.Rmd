...
BGSIZE=length(which(detgenes %in% reactomegenes))
...
barnames <- gsub("_"," ",gsub("REACTOME_","",rownames(top)))
cols <- gsub("TRUE","red",gsub("FALSE","blue",as.character(top$s.dist>0)))
par(mar = c(6.1, 18.1, 1.1, 0.5))
barplot(abs(top$s.dist),horiz=TRUE,las=1,names.arg=barnames, cex.names=0.4,
  cex.axis=0.5,col=cols)
