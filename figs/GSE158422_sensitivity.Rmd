...
nsigs <- lapply(sizes, function(z) {
  ...
  nsig <- nrow(subset(rbind(gsaup,gsadn),FDR<0.05))
  return(nsig)
})
nsigs <- unlist(nsigs)
names(nsigs) <- sizes
par( mar = c(5.1, 6.1, 4.1, 2.1) )
barplot(nsigs,horiz=TRUE,las=1,xlab="no. significant sets")
...
rownames(gsadn) <- paste(rownames(gsadn),"DN")
ngsig <- nrow(subset(rbind(gsaup,gsadn),FDR<0.05))
