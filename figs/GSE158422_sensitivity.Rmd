...
myann <- data.frame(anno[,c("GencodeCompV12_NAME","Regulatory_Feature_Group","Islands_Name","Relation_to_Island")])
...
agg <- function(dm,cores=1) {
  dm <- dm[which(dm$GencodeCompV12_NAME!=""),]
  gn <- unique(unlist(strsplit( dm$GencodeCompV12_NAME ,";")))
  gnl <- strsplit( dm$GencodeCompV12_NAME ,";")
  gnl <- mclapply(gnl,unique,mc.cores=cores)
  dm$GencodeCompV12_NAME <- gnl
...
length(unique(unlist(strsplit(subset(dm,adj.P.Val<0.05 & logFC>0)$GencodeCompV12_NAME,";"))))
length(unique(unlist(strsplit(subset(dm,adj.P.Val<0.05 & logFC<0)$GencodeCompV12_NAME,";"))))

length(unique(unlist(strsplit(dm$GencodeCompV12_NAME,";"))))
...
detgenes <- unique(unlist(strsplit(dm$GencodeCompV12_NAME,";")))
reactomegenes <- unique(unlist(gs_symbols))
length(reactomegenes)
BG=detgenes[(which(detgenes %in% reactomegenes))]
BGSIZE=length(BG)
FGUP <- unique(unlist(strsplit(subset(dm,logFC > 0 & adj.P.Val < 0.05)$GencodeCompV12_NAME,";")))
FGUP <- FGUP[which(FGUP %in% reactomegenes)]
FGDN <- unique(unlist(strsplit(subset(dm,logFC < 0 & adj.P.Val < 0.05)$GencodeCompV12_NAME,";")))
...
gsres <- lapply(SAMPLESIZES,function(n) {
  ...
  dsres <- mclapply(SEEDS, function(SEED) {
    ...
    if ( is.na(dm2$logFC[1]) ) {
      ...
      sigup2 <- rownames(subset(dm2,adj.P.Val<0.05 & GencodeCompV12_NAME != "" & logFC>0))
      if ( length(sigup2) < 250 ) { sigup2 <- head(rownames(subset(dm2,logFC>0 & GencodeCompV12_NAME != "" )),250) }
      sigdn2 <- rownames(subset(dm2,adj.P.Val<0.05 & GencodeCompV12_NAME != "" & logFC<0))
      if ( length(sigdn2) < 250 ) { sigdn2 <- head(rownames(subset(dm2,logFC<0 & GencodeCompV12_NAME != "" )),250) }
