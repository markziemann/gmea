...
false positives.
A better solution could be to select random gene sets with sizes similar to Reactomes.
Reactomes were downloaded on the 14th Sept 2023 from MsigDB.
...
Get a vector of genes, and the length of gene sets from Reactome.

Then generate random gene sets.

```{r,gset_prep}

gene_catalog <- unique(unlist(strsplit(myann$UCSC_RefGene_Name,";")))

lengths <- unname(unlist(lapply(gs_symbols,length)))
...
simgsa <- function(genesetdatabase, myann, mval, seed, frac_genes, frac_probes, groupsize, delta=1, num_dm_sets=50) {
  ...
  gset_mod <- gset_selection(genesetdatabase=gsets,seed=seed,gene_catalog=gene_catalog)
  # cut down length to num_dm_sets
  set.seed(seed) ; gset_mod <- sample(gset_mod,num_dm_sets)
  ...
  gene2entrez <- mapIds(org.Hs.eg.db, gene_catalog, 'ENTREZID', 'SYMBOL')
...
simla <- function(genesetdatabase, myann, mval, seed, frac_genes, frac_probes, groupsize, delta=1, num_dm_sets=50) {
  ...
  lares1 <- ttenrich(m=m1,genesets=gsets,cores=10)
...
simal <- function(genesetdatabase, myann, mval, seed, frac_genes, frac_probes, groupsize, delta=1, num_dm_sets=50) {
  ...
  colnames(m1) <- "t"
  alres1 <- ttenrich(m=m1,genesets=gsets,cores=10)
  # summarise results
...
genesetdatabase=gs_symbols
groupsize=10
delta=1
num_dm_sets=50

aares <- mclapply(1:6,function(i) {
  simaa(genesetdatabase=gs_symbols, myann=myann, mval=normal_mval, seed=i*100, frac_genes=0.5, frac_probes=0.5,
    groupsize=10, delta=delta, num_dm_sets=num_dm_sets)
},mc.cores=6)

aares <- do.call(rbind,aares)
aares
...
almres <- mclapply(1:6,function(i) {
...
gres <- lapply(1:6,function(i) {
  simgsa(genesetdatabase=gs_symbols, myann=myann, mval=normal_mval, seed=i*100, frac_genes=0.5, frac_probes=0.5,
    groupsize=10, delta=delta, num_dm_sets=num_dm_sets)
})
