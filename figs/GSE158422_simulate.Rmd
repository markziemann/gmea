...
A requirement is that there will be no overlap between them.
...
gset_selection <- function(genesetdatabase,seed,gene_catalog) {
  set.seed(seed)
  gsets <- gs_symbols[sample(1:length(genesetdatabase))]
  genelist=NULL
  genesets=NULL
...
gset_mod <- gset_selection(genesetdatabase=gs_symbols,seed=100,gene_catalog=gene_catalog)
...
incorp_dm <- function(genesetdatabase,myann,gene_catalog,mval,seed,frac_genes,frac_probes,groupsize) {
  # select gene sets to alter
  gset_mod <- gset_selection(genesetdatabase=genesetdatabase,seed=seed,gene_catalog=gene_catalog)
  # divide gene sets between hyper and hypomethylated
  nset <- floor(length(gset_mod)/2)
  set.seed(seed) ; gtup <-sample(gset_mod,nset)
  set.seed(seed) ; gtdn <- sample(setdiff(gset_mod,gtup),nset)
  ...
  set.seed(seed) ; pdn2 <- sample(pdn,floor(length(pup)*frac_probes))
  ...
  mval_case[rownames(mval_case) %in% pup2,] <-  mval_case[rownames(mval_case) %in% pup2,] +1
  mval_case[rownames(mval_case) %in% pdn2,] <-  mval_case[rownames(mval_case) %in% pdn2,] -1
...
simgsa <- function(genesetdatabase,myann,gene_catalog,
  mval,seed,frac_genes,frac_probes,groupsize) {

  sim <- incorp_dm(genesetdatabase=gs_symbols,myann=myann,gene_catalog=gene_catalog,
    mval=mval,seed=seed,frac_genes=0.5,frac_probes=0.5,groupsize=groupsize)
  mval2 <- sim$mval
  ncols <- ncol(mval2)
  groupsize <- ncols/2
  ...
  gsaup3 <- gsameth(sig.cpg=pup3, all.cpg=rownames(dm3), collection=gs_entrez)
  gsadn3 <- gsameth(sig.cpg=pdn3, all.cpg=rownames(dm3), collection=gs_entrez)
...
gres <- mclapply(1:50,function(i) {
  simgsa(genesetdatabase=gs_symbols,myann=myann,gene_catalog=gene_catalog,
    mval=normal_mval,seed=i*100,frac_genes=0.5,frac_probes=0.5,groupsize=groupsize)
},mc.cores=10)
