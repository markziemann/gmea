...
myfiles <- list.files("hillary",pattern="prevalent_full",full.names=TRUE)
l1 <- lapply(myfiles, read.csv)
...
tic()
m <- mitch_import(x=l1,DEtype="prescored",geneTable=gt,geneIDcol="CpG")
toc() #117 sec
head(m)
...
tic()
mres <- mitch_calc(x=m,genesets=gs_symbols,minsetsize=5,cores=8, priority="effect")
toc() #21.3s
...
```{r,mitch2,fig.height=5,fig.width=5}
...
```{r,mitchheat,fig.height=9,fig.width=9}

top <- mtable[which(mtable$set %in% gsets),]
...
#colfunc <- colorRampPalette(c("#B80F0A", "white", "#1134A6"))
