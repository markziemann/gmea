...
SIG = nrow(sig)
UP = nrow(subset(sig,ES>0))
DN = nrow(subset(sig,ES<0))

plot(fgseaRes$ES,-log10(fgseaRes$pval))
points(sig$ES,-log10(sig$pval),col="red")
mtext(paste(SIG,"signifiant genes,",UP,"up,",DN,"down"))
...
Need to add volcano and nprobe charts here.
...
rownames(cres) <- sub("\\.","@", rownames(cres))
...
plot(cres$median,-log10(cres$PValue))
points(sig$median,-log10(sig$PValue),col="red")

plot(cres$NGenes,-log10(cres$PValue),log="x")
points(sig$NGenes,-log10(sig$PValue),col="red")

```

## GMEA using ROAST test

ROAST is self contained test using rotations.


## TODO

The goal is to use a self contained test that can handle extensive correlation.

Globaltest, ANCOVA and SAM-GS

GSVA

Q2 (Tian et al. [14])
14. Tian L, Greenberg SA, Kong SW, et al. Discovering statistically significant pathways in expression profiling studies. Proc Natl Acad Sci USA. 2005;102(38):13544–9. [PMC free article] [PubMed] [Google Scholar]

FCS.SC—self-contained version of FCS (Pavlidis et al. [24])
24. Pavlidis P, Qin J, Arango V, et al. Using the gene ontology for microarray data mining: a comparison of methods and application to age effect in human prefrontal cortex. Neurochem Res. 2004;29(6):1213–22. [PubMed] [Google Scholar]
