...
detect altered methylation before a diagnosis has been made.
...
These analyses also consistently showed that non-parametric methods showed better recall with only a small decrease in precision, in accordance with some previous reports.
Summarisation to gene level only takes a few seconds with the efficient base R aggregation command,
...
make use of multi-threaded processors, so it is not much slower than conducting ORA with existing methods.

* But it was interesting that ORA performed better when gene sets are smaller.
Given that KEGG and Reactome pathways consist of both large (>100) and small (<20) gene sets,
a hybrid FCS and ORA approach might increase sensitivity and recall.

* At 100 genes per set, LAM had superior sensitivity compared to GSA, which agrees with mitch's performance with simulated RNA-seq gene
expression data[@Kaspi2020-yx].
The improved sensitivity of this method provides a new opportunity for researchers to reanalyse previously conducted EWASs with contemporary pathway databases to better understand
the subtle signatures, as we have demonstrated with the chronological age and assisted reproduction examples.
The analyses conducted here used Reactome pathways, but this method is general purpose, so different gene sets can be used to examine various hypotheses.
...
These results indicate that there is a complicated relationship between DNA methylation and gene expression in lung cancer and further work is required to appreciate the mechanisms involved.
...
Using this example, generating similar gene tables for the new EPIC 2.0 array and any future arrays will be straightforward, once their annotation sets are available through Bioconductor.
...
We note that other packages for enrichment of Infinium array data to not by default address this, which could lead to a loss of these genes from pathway enrichment and a loss of sensitivity.
...
On contrast, LAM identified
Such work is important as it is known that methylation changes can be caused by disease states.
