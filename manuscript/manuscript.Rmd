...
Infinium methylation bead arrays remain one of the most popular platforms for studying genomic DNA methylation,
but tools for downstream pathways analysis have their limitations.
Functional class scoring (FCS) is a group of pathway enrichment techniques that involve the ranking of genes
and evaluation of their collective regulation in biological systems, but the implementations described for
...
This method, which we call “LAM”, outperformed an existing over-representation analysis method in simulations,
and showed higher sensitivity and robustness in an analysis of real lung tumour-normal paired datasets.
Using matched RNA-seq data we examine the relationship of methylation differences at promoters and gene bodies
with RNA expression at the level of pathways in lung cancer.
...
existing methods.
A reproducible vignette is provided to illustrate how to implement this method.
...
regulation score (eg: fold change, confident effect size, t-statistic) followed by a test to assess whether each
...
limitations.
Since genes have a variable number of probes, there are many concevable ways that FCS could be applied
to Infinium methylation array data.
The first set of methods (type I) use differential probe methylation data from `limma` [@Ritchie2015-zn] as an input.
...
and end with additional examples related to human ageing, assisted reproductive technologies and a large-scale EWAS of
19 diseases.
...
Here, we outline eight potential approaches for FCS of Infinium methylation data, where methods 1-5 are type I
and 6-8 are type II:
...
limma differential methylation analysis, followed by enrichment analysis using the `GSAmeth` approach, or one of
...
with a condition.
To this end, we examined differential pathway methylation associated with incidence of 19 disease states.
...
Updating defunct gene symbols is an important step as 2,788 and 3,253 gene symbols were updated on the HM450K and EPIC array annotations, which represents approximately 10-14% of genes on these platforms.
...
Epigenetic differences may also contribute to disease risk.
