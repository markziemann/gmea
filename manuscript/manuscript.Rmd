...
Severine Lamon<sup>3</sup>,
Mary Sravya Kasu<sup>1</sup>,
Mitch W. Norris<sup>1</sup>,
Yen Ting Wong<sup>4</sup>,
Jeffrey Craig<sup>4,5</sup>
</strong>
...
| Severine Lamon | 0000-0002-3271-6551 |
...
Infinium methylation beadarrays remain one of the most popular platforms for studying genomic
DNA methylation, but tools for downstream pathways analysis have their limitations.
Functional class scoring (FCS) is a group of pathway enrichment techniques that involve the ranking of
genes and evaluation of their collective regulation in biological systems, but the implementations described for
Infinium methylation data do not retain direction information which is important for mechanistic understanding
of genomic regulation.
Here, we evaluate several candidate FCS methods that retain directional information.
According to simulation results, the best performing method involves the mean aggregation of probe limma
t-statistics by gene followed by a rank-ANOVA enrichment test using the mitch package.
This method, which we call "LAM", outperformed an existing over-representation analysis method in
simulations, and showed higher sensitivity and robustness in an analysis of real lung tumor-normal paired datasets.
Using matched RNA-seq data we examine the relationship of methylation differences at promoters and
gene bodies with RNA expression at the level of pathways in lung cancer.
To further demonstrate the utility of our approach, we apply it to two other contexts; firstly an examination of
methylation differences associated with chronological age; and secondly to examine methylation differences in
infants conceived with in vitro fertilisation.
These results indicate that the LAM method is more sensitive and precise as compared to existing methods.
LAM has the potential to illuminate disease mechanisms and identify novel diagnostic signatures.
A vignette is provided to illustrate how to implement this method.
...
set of genes has a distribution of scores that is different to the null [@Subramanian2005-ar].
The `gsameth()` function of the `missMethyl` package is the state-of-the-art method for ORA of infinium methylation array
data as it addresses issues related to probes belonging to more than one gene and the fact that genes can have multiple
probes [@Maksimovic2021-ur].
As gsameth is an ORA method, results dependent strongly on the significance threshold used [@Ren2019-mr], and also
...
direction of methylation change, it limits the utility for downstream interpretation of the importance of methylation
...
limitations.
Due to the fact that genes have a variable number of probes, there are many concevable ways that FCS could be applied
to Infinium methylation array data.
The first set of methods use differential probe methylation data from `limma` [@Ritchie2015-zn] as an input.
The differential methylation of probes beloning to each gene are scored and these gene scores
undergo an enrichment test.
The second set of methods first aggregate probe methylation values for each gene before differential
methylation analysis with limma.
Then, differential gene methylation results are used for a downstream enrichment test.
With the above methods there are several methodological choices, such as the type of aggregation (eg: mean, median), and the type of
downstream test (eg: parametric or nonparametric).
...
We then examine the sensitivity of selected methods on real cancer data, investigate the association with gene expression and end
with additional examples related to human aging and assisted reproductive technologies.
...
Here, we outline eight potential approaches for FCS of Infinium methylation data:
...
Similar to LAW, however instead of the Wilcoxon test, the mitch package[@Kaspi2020-yx] is used to execute a two-way,
two-sample ANOVA-on-ranks test (Kruskal-Wallis).

5. Limma Rank Mitch (LRM).
Probes are ranked by t-statistic. The mean rank values for probes beloning to each gene are computed and used
...
Simulations were conducted on a SLURM cluster [@Yoo2003-mc].
...
Downstream pathway enrichment analysis was conducted using the GSAmeth and LAM methods
...
annotation.
As gene annotations are several years old, we used HGNChelper to update them (database current as of 21-Dec-2023) [@Oh2022-ts]
(3253 gene symbols were updated).
...
This process was repeated 50 times at each sample size.

### Association of methylation with gene expression

RNA-seq based gene counts for the same set of lung cancer patients were downloaded from GEO (accession: GSE158420).
...
In order to deminstrate the utility of the LAM method for large EWAS studies, we examined
methylation profile associations with age of two independent cohorts involving a total of 7036
...
Briefly, gene-probe associations were first obtained from the "IlluminaHumanMethylationEPICanno.ilm10b4.hg19"
dataset, followed by update of gene symbols with HGNChelper.
...
independent studies.
The Estill study was published in 2016 and is based on the HM450K array and the data is available from GEO Accession GSE79257
[@Estill2016-jh].
...
GSE131433[@Novakovic2019-jt].
Various assisted reproductive technology conception groups are described in these studies, but we examined
just fresh in vitro fertilisation (IVF) conceived infants compared to naturally conceived infants.
Raw intensity files (IDAT format) were read into R with minfi [@Aryee2014-su], normalistion was conducted with the SWAN method
...
Probe gene associations for HM450K array were established using the "IlluminaHumanMethylation450kanno.ilmn12.hg19" annotation set
and the gene symbols were updated with HGNChelper.
As the Novakovic study involved EPIC array data, gene-probe associations were obtained from the
"IlluminaHumanMethylationEPICanno.ilm10b4.hg19" annotation set, and gene symbols were updated with
HGNChelper.
...
The study used a logitudinal design, with sample collection baseline and diagnosis status established via electronic health
...
Finally, as a test of type I errors, we randomised each of the 19 incident profiles using
...
Simulations were conducted using the eight FCS methods (LAT, LTT, LAW, LAM, LRM, ALT, ALW, ALM) and GSAmeth,
...
There were 30 pathways that were common between LAM and GSA methods.
...
This may seem counterintuitive, but may be due to LAM's better sensitivity in dectecting pathway enrichment when effect sizes are
lower.
...
(C) Significant LAM pathways with largest S distance, a proxy for effect size.
(D) Top ranked significant GSA pathways ranked by fold enrichment.](img/fig3.svg)
...
This process was repeated 50 times at each sample size and the results shown in (**Figure 4**).
...
group.
Left panels correspond to LAM, and right panels to GSA methods.](img/fig4.svg)
...
In promoters, GSA identified 15 and 7 pathways with higher and lower methylation respectively, while at gene bodies, there were were
...
Top 20 gene sets shown with largest s distance after FDR filtering at 0.05.](img/fig5.svg)
...
indicates 268 pathways with higher methylation in the discovery sample and lower in methylation in the replication
sample (**Figure 6B**).
When prioritising the results by S distance, a surrogate measure of effect size, 29 of the top 30 pathways were concordant in their
direction of regulation (**Figure 6C**).
Some of the observed methylation changes make sense with reports in the literature.
For example the complement pathway is associated with aging and in this analysis we observe a reduction in methylation of genes
beloning to this set including *C1R*, *MBL2*, *C1S* and *C1QC* (**Figure 6D**).
...
Pathways with MANOVA FDR<0.05 are shown in red while others are shaded gray.
...
in both discovery and replication studies.](img/fig6.svg)
...
Two of the highest powered studies with publicly available data are Estill 2016 which used the 450K array and Novakovic 2019 study
...
embryo transfer.
The Estill dataset on the 450k platform had 418,833 probes that met the quality criteria (Natural n=43; IVF n=38).
Of these, 149,589 probes showed statistical significant differential methylation (FDR<0.05), of which 28,767 and 120,822 exhibited
...
Of these, 5,562 were statistically significant (FDR<0.05) with 1,348 and 4,214 exhibited higher or lower methylation in the IVF group
respectively.
After summarisation to the gene level, the Estill dataset on 450k array describes 19,240 genes, while the Novakovic dataset on EPIC
...
trend of more genes having lower methylation level in the IVF group (**Figure 7A**).
...
(A) Contour heatmap showing the similarity in gene methylation score ranks in the Estill (450k) and Novakovic (EPIC) studies.
...
member genes in both Estill and Novakovic studies.](img/fig7.svg)

## Differential pathway methylation in 19 common disease states

To examine the potential for LAM to illuminate differential pathway methylation associations
with diseases, we obtained and analysed blood based EWAS summary statistics from a recent
publication [@Hillary2023-mu].
This study examined 14 prevalent disease states and incidence of 19 disease states in a group
...
the fewest (3).
We selected up to five statistically significant pathways with an absolute S distance of >0.4 in either
...
(A) A barplot showing the number of statistically significant pathways with higher and lower methylation identified in each prevalent condition (FDR<0.05).
...
with a condition.
So we examined differential pathway methylation associated with incidence of 19 disease states.
Over all 19 conditions, there were 1570 significant pathways (FDR<0.05) (**Figure 9A**).
Liver cirrhosis has the most, with 464, followed by COPD with 400, while Alzheimer's has just one and COVID-19
...
(A) A barplot showing the number of statistically significant pathways with higher and lower methylation identified in each incident condition (FDR<0.05).
...
Across the 100 repeats, the mean number of false positives was 1.95 and median was 0.
...
Overall, we found that the FCS methods performed better across all simulation conditions.
...
for each sample for each gene prior to limma.
...
Our analyses also consistently showed that non-parametric methods showed better recall with only a small decrease in precision, in accordance with some previous reports.
...
An interesting observation is that ORA performed better when gene sets are smaller.
...
At 100 genes per set, LAM had superior sensitivity compared to GSA, which agrees with a prevous report of mitch's performance with
simulated RNA-seq gene expression data[@Kaspi2020-yx].
The improved sensitivity of the LAM method provides a new opportunity for researchers to reanalyse previously conducted EWASs with contemporary pathway databases to better understand
the subtle signatures, as we have demonstrated with the chronological age, assisted reproduction examples and 19 disease EWAS analyses.
...
The differences in pathway results obtained with LAM and GSA from lung cancer data were striking, not only in the numbers of pathways identified, but also their relevance to the disease.
...
Joint enrichment analysis showed some genes with increasing promoter methylation together with increasing RNA expression (eg: "Negative regulation of AP2 transcription factors").
Likewise a few pathways showed decreases in methylation with decreases in RNA expression such as "Dectin family", "TLR regulation" and "glycan biosynthesis termination".
These results indicate that there is a complicated relationship between DNA methylation and gene expression in lung cancer and further work is required to appreciate the mechanisms involved and distinguish which CpGs are critical for influencing gene expression and which are not.

We have demonstrated that this method works with both HM450K and EPIC array data and can even facilitate the comparison of EWAS using different platforms.
We have provided an example workflow that researchers can use as a template.
It shows how to generate a probe-gene table from Bioconductor annotation datasets, how to update the defunct gene names and how to run the enrichment analysis.
Using this example, generating similar gene tables for the new Infinium Mouse and EPIC 2.0 arrays and any future arrays will be straightforward, once their annotation sets are available through Bioconductor.

Updating defunct gene symbols is an important step as 2,788 and 3,253 gene symbols were updated on the HM450K and EPIC array annotations, which is approximately 10-14% of genes on these platforms.
We note that other packages for enrichment of Infinium array data do not by default address this, which could lead to a loss of these genes from pathway enrichment and diminished sensitivity.

The analysis of the large scale disease EWAS shows that LAM can be readily applied to identify differential pathway methylation associated with prevalent and incident disease.
The authors of the study used GOmeth for their pathway analysis and from the 33 models examined, significant findings were obtained for only two, prevalent diabetes and heart disease.
On contrast, LAM identified 2,469 pathway associations (74.8 per model).
To demonstrate these are not the results of false positives, permutated profiles yielded on average 0.10 significant findings per model,
supporting the idea that LAM is identifying real pathway signatures.

Distinguishing prevalent and incident signatures is important as it is known that methylation changes can be caused by disease states.
For example metabolic changes in diabetes are probably the cause of the observed methylation signatures.
It is perhaps unexpected that more significant pathways be identified from the incident disease arm of the study,

This work could be extended to include not just proximal CpG sites, but also enhancer based probes and their target genes based on resources such as GeneHancer.

There is also potential to generate new libraries of differentially methylated probe and gene sets, to contribute to the pool of molecular signatures in public resources like MSigDB,
...
(https://hub.docker.com/repository/docker/mziemann/gmea/general).
...
We thank the researchers who publicly shared data to enable this research.
